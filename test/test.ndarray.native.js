...
var dsem = tryRequire( resolve( __dirname, './../lib/ndarray.native.js' ) );
var opts = {
...
tape( 'main export is a function', opts, function test( t ) {
	...
	t.strictEqual( typeof dsem, 'function', 'main export is a function' );
	t.end();
});

tape( 'the function has an arity of 5', opts, function test( t ) {
	t.strictEqual( dsem.length, 5, 'has expected arity' );
	t.end();
});
...
tape( 'if provided an `N` parameter equal to `1`, the function returns a standard error of the mean of `0` (population)', opts, function test( t ) {
	var x;
	var v;
...
tape( 'if provided a `correction` parameter yielding `N-correction` less than or equal to `0`, the function returns `NaN`', opts, function test( t ) {
	...
	v = dsem( x.length, x.length, x, 1, 0 );
	t.strictEqual( isnan( v ), true, 'returns expected value' );

	v = dsem( x.length, x.length+1, x, 1, 0 );
	t.strictEqual( isnan( v ), true, 'returns expected value' );
